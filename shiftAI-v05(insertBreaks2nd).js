...
function testFunction01() {
	...
	shiftNum++; 
	makeShiftPattern(12, 19, shiftNum);
	shiftNum++;
...
function addBreakToShift(numberOfShifts){
	...
	for (i = 0; i <= numberOfShifts; i++) {
		...
		document.getElementById("myTable").appendChild(y1);
		var z = document.createElement("TD");
		var t = document.createTextNode("s#" + i + " ==> " + arrayOfShiftAll[i][34]);
		z.appendChild(t);
		...
		if (arrayOfShiftAll[i][39] == 0) { 
			...
			for (j = 0; j < 34; j++) {
				...
				if(arrayOfShiftAll[i][37] <= hrsPattern[j] && arrayOfShiftAll[i][38] > hrsPattern[j]) {	
					...
					//
					if ((counterLLB == 0 || counterLLB == 1) &&
						((findFirst3rdForBreak == hrsPattern[j]-0.5) || (findFirst3rdForBreak == hrsPattern[j]))) {
							arrayOfShiftAll[i][j] = "L";
							var t = document.createTextNode("L");
							counterLLB++;
							if(counterLLB == 2){c = false;}
					//counterLLB == 2 || counterLLB == 3	
					} else if((c) &&
								((findFirst3rdForBreak == hrsPattern[j]-1.5) || (findFirst3rdForBreak == hrsPattern[j]-1))) {
							arrayOfShiftAll[i][j] = "L";
							var t = document.createTextNode("L");
							counterLLB++;
							if (counterLLB == 4) {
